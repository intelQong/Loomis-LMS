# Loomis LMS — Cloudflare Pages + D1 Deployment
...
5. Choose `create_database=true` the first time to create `loomis_lms`.
...
```bash
npx wrangler d1 create loomis_lms
```
...
```bash
npx wrangler d1 migrations apply loomis_lms --local
```

For production Cloudflare D1:

```bash
npx wrangler d1 migrations apply loomis_lms --remote
```
...
```bash
npx wrangler d1 execute loomis_lms --remote --command "UPDATE users SET role='admin', status='active' WHERE email='YOUR_EMAIL@example.com';"
```
...
```bash
npx wrangler d1 execute loomis_lms --remote --command "UPDATE users SET role='faculty', status='active' WHERE email='FACULTY_EMAIL@example.com';"
```

3. Find the faculty UID:

```bash
npx wrangler d1 execute loomis_lms --remote --command "SELECT id, email FROM users WHERE role='faculty';"
```
...
```bash
npx wrangler pages dev . --d1 DB=loomis_lms
```
...
7. Add binding:
   - Variable name: `DB`
   - D1 database: `loomis_lms`
8. Redeploy after adding the binding.

### Option B: Wrangler Direct Deploy

```bash
npx wrangler pages deploy . --project-name loomis-lms
```
...
- Sessions use an HTTP-only cookie named `loomis_session`.
