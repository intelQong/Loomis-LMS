...
export async function onRequest(context) {
  try {
    ...
    if (path[0] === 'students' && path[1] === 'bulk-schedule' && method === 'POST') return updateStudentSchedules(context, user);
...
async function updateStudentSchedules({ request, env }, user) {
  requireRole(user, ['admin']);
  await ensureUserCompatibilityColumns(env);
  const body = await readJson(request);
  const studentIds = Array.isArray(body.studentIds)
    ...
    : [];
  const classDays = body.classDays === undefined ? null : String(body.classDays || '').trim();
  const classTime = body.classTime === undefined ? null : String(body.classTime || '').trim();

  if (studentIds.length === 0) throw httpError('Select at least one student to update.', 400);
  if (studentIds.length > 500) throw httpError('Please update 500 or fewer students at a time.', 400);
  if (!classDays && !classTime) throw httpError('Choose class days or class time to update.', 400);

  const stmts = studentIds.map(id => {
    if (classDays && classTime) {
      return env.DB.prepare("UPDATE users SET class_days = ?, class_time = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND role = 'student'")
        .bind(classDays, classTime, id);
    }
    if (classDays) {
      return env.DB.prepare("UPDATE users SET class_days = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND role = 'student'")
        .bind(classDays, id);
    }
    return env.DB.prepare("UPDATE users SET class_time = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND role = 'student'")
      .bind(classTime, id);
  });
  ...
  await logAction(
    ...
    `Updated class schedule for ${updated} students. Days: ${classDays || 'unchanged'}; Time: ${classTime || 'unchanged'}`
  );
